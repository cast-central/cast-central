...
function list(req, res){
    ...
    protocol(search, function(chromecasts){
        debug(chromecasts);
        res.json({'chromecasts': chromecasts});
    });
}

// Given an address, port and app name for the chromecast, 
// launch will connect to the chromecast given and try to 
...
function launch(req, res){
    ...
    is_defined({'name': name, 'address': address, 'port': port, 'app': app}, res);
    ...
    if(typeof players[name] === 'undefined'){
        ...
        res.json({'success': false, 'message': 'chromecast already in use'});
    }
}
